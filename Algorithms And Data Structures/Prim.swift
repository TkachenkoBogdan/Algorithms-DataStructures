...
//  Alg&DataStr
//
//  Created by Богдан Ткаченко on 7/9/19.
...
public class Prim<T: Hashable> {
    ...
    public func produceMinimumSpanningTree(for graph: Graph)
        -> (cost: Double, mst: Graph) {
            ...
            addAvailableEdges(for: start,
                              in: graph, check: visited, to: &priorityQueue)
            ...
            while let smallestEdge = priorityQueue.dequeue() {
                ...
                mst.add(.undirected,
                        from: smallestEdge.source, to: smallestEdge.destination, weight: smallestEdge.weight)
    ...
    internal func addAvailableEdges( for vertex: Vertex<T>, in graph: Graph,
